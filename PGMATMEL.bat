...
rem ISP programmer. 
rem (This can be overridden on the command line with /PCOMx where 'x' is 1..255)
rem
SET PORT=COM4
rem
...
rem the names below to the correct section names.  
rem This batch file programs the sections mentioned in the order listed. 
rem (Note: The the order of fuses in the _F.txt file is the order shown here)
rem Up to 6 sections can be listed here.  List the 'lock' section last. 
rem (Sections up to 'lock' will be read, all sections may be written)
rem 
...
Echo AVRDUDE and either the ProMicro ISP programmer or the Arduino ISP 
Echo programmer.  It uses (if programming) or creates (if reading) up to 
Echo three different Intel hex files (and command line fuse options) to then 
Echo call AVRDude, after which it then processes the results. 
Echo:
...
Echo                  If no /P option then use the Arduino ISP programmer. 
Echo:
...
Echo AVRDUDE and either the ProMicro ISP programmer or the Arduino ISP 
Echo programmer.  It uses (if programming) or creates (if reading) up to 
Echo three different Intel hex files and command line options to then call 
Echo AVRDude, after which it then processes the results. 
Echo:
...
if "%EX%" == "0" goto :EOF
if "%PORTST%" == "#" goto :EOF
mode %PORTST% baud=19200 data=8 parity=N > NUL 2> NUL
echo: > %PORTST%
goto :EOF
rem 
rem 
...
SET DUDEHW=arduinoisp&& rem hardware to use ... arduinoisp or arduino
...
if /I "%TU:~1,1%"=="P" SET PORTST=%TU:~2%&&   goto CMDLINENEXT
...
if /I "%TU:~0,2%"=="--" SET DUDEOPTS1=%DUDEOPTS1% %TV:~1%&& goto CMDLINENEXT 
if /I "%TU:~0,1%"=="-"  SET DUDEOPTS2=%DUDEOPTS2% %TV%&&    goto CMDLINENEXT 
rem
...
rem PORTST could be # (don't touch DUDEHW), or blank (use arduino -PDfltPort) 
rem or a value (set DUDEHW to arduino -PPORTST)
if "%PORTST%"=="#" goto CMDLNDONE2
if "%PORTST%"=="" SET PORTST=%PORT%
SET DUDEHW=arduino -P%PORTST%
...
rem Set sections to one space (don't "cleanup")
SET MAXSECS=6
...
:SECLOOP
  ...
  SET NWSEC=%x%
  if /I "%name%" == "lock"  SET y=1
  if "%y%"=="0" SET NRSEC=%x%
  if [%x%] == [%MAXSECS%] goto SECLOOPDONE
  SET /a "x+=1"
  goto SECLOOP
:SECLOOPDONE
rem echo Number of sections is W='%NWSEC%' R='%NRSEC%'
rem
...
:FCADDLOOP
  SET /a "x+=1"
  rem get the name, value and dude section name for the current fuse
  SET name=FUSE%x%
...
Echo:
Echo Command line for AVRDude is:
Echo -C"%DUDECFG%" %DUDEQUIET% -c%DUDEHW% -p%DEVICE% %DUDEOPTS1% %FLASHCMD% %EECMD% %FUSECMD% %DUDEOPTS2% 
Echo:
SET EX=0
GOTO PROGRAMMINGDONE
:RUNDUDE
...
CALL :CKSETERRLITE
if "%EX%" == "0" goto PROCESSFUSESREAD
if "%PORTST%" == "#" goto PROCESSFUSESREAD
mode %PORTST% baud=19200 data=8 parity=N > NUL 2> NUL
echo: > %PORTST%
rem
rem ****************************************************************************
